def show_menu():
    print('\n*******************************\n'
          '|Выберите нужный пункт меню: ↓|\n'
          '|-----------------------------|\n'
          '|1. Показать телефонную книгу |\n'
          '|-----------------------------|\n'
          '|2. Загрузить телефонную книгу|\n'
          '|-----------------------------|\n'
          '|3. Сохранить телефонную книгу|\n'
          '|-----------------------------|\n'
          '|4. Добавить контакт          |\n'
          '|-----------------------------|\n'
          '|5. Изменить контакт          |\n'
          '|-----------------------------|\n'
          '|6. Удалить контакт           |\n'
          '|-----------------------------|\n'
          '|7. Найти контакт             |\n'
          '|-----------------------------|\n'
          '|0. Выход из программы        |\n'
          '|_____________________________|\n')
    return user_choice()


def user_choice():
    while True:
        try:
            choice = int(input('>>> '))
            if choice >= 0 and choice <= 7:
                return choice
            else:
                print('Такого пункта в меню нет, выберите повторно.')
        except:
            print('Введено некорректное значение!')


def print_phone_book(phone_book: list):
    if len(phone_book) > 0:
        for id, contact in enumerate(phone_book, 1):
            print(id, *contact)
    else:
        print('Телефонная книга не загружена или пуста. Попробуйте загрузить телефонную книгу.')


def exit():
    print('Работа программы завершена. До свидания.')


def load_successful():
    print('Загрузка телефонной книги выполнена успешно.')


def save_successful():
    print('Сохранение телефонной книги выполнено успешно.')


def remove_successful():
    print('Удаление контакта выполнено успешно.')


def new_contact():
    name = input('Введите ФИО контакта: ')
    phone_number = input('Введите номер контакте: ')
    comment = input('Введите комментарий для контакта: ')
    return [name, phone_number, comment]


def remove_choice():
    try:
        id = int(input('Введите ID номер контакта для удаления: '))
        return id
    except:
        print('Введены некорректные данные! Укажите ID контакта.')
